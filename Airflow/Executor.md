...
Executorii sunt mecanismul prin care instanțele de sarcină sunt executate. Aceștia au un API comun și sunt "pluggable", ceea ce înseamnă că poți schimba executorii în funcție de nevoile instalației tale.
...
Există două tipuri de executori - cei care `rulează sarcinile local` (în interiorul procesului de planificator) și cei care rulează `sarcinile lor la distanță` (de obicei printr-un grup de `workeri`). 

Airflow vine configurat implicit cu `SequentialExecutor`, care este un `executor local` și cea mai simplă opțiune pentru execuție. Cu toate acestea, `SequentialExecutor` nu este potrivit pentru producție, deoarece nu permite rularea sarcinilor în paralel și din acest motiv, unele caracteristici Airflow (de exemplu, rulează senzori) nu vor funcționa corect. Ar trebui să folosești în schimb `LocalExecutor` pentru instalații de producție mici, pe o singură mașină, sau unul dintre executorii la distanță pentru o instalare multi-mașină/cloud.
...
- Ai o unealtă/serviciu privat pentru executarea sarcinilor care este disponibil doar pentru tine sau organizația ta.
...
| Metodă             | Descriere                                                                                                                                                     |
|--------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `heartbeat`        | Ciclul de lucru al Job-ului planificatorului Airflow va apela periodic heartbeat pe executor. Această metodă actualizează metrici, declanșează execuția sarcinilor noi și actualizează starea sarcinilor în execuție/terminate.                  |
...
| `has_task`         | Planificatorul folosește această metodă BaseExecutor pentru a determina dacă un executor are deja o instanță de sarcină specifică așteptată sau în execuție.      |
...
`sync`: Metoda `sync` va fi apelată periodic în timpul bătăilor de inimă ale executorului. Implementează această metodă pentru a actualiza starea sarcinilor pe care executorul le cunoaște. Opțional, încearcă să execute sarcini așteptate care au fost primite de la planificator.

`execute_async`: Execută o comandă în mod asincron. O comandă în acest context este o comandă CLI Airflow pentru a rula o sarcină Airflow. Această metodă este apelată (după câteva straturi) în timpul bătăilor de inimă ale executorului care rulează periodic de către planificator. În practică, această metodă adaugă adesea sarcini într-o coadă internă sau externă de sarcini de rulat (de exemplu, `KubernetesExecutor`). Dar poate, de asemenea, să execute sarcinile direct (de exemplu, `LocalExecutor`). Acest lucru va depinde de executor.
...
| `end` | Job-ul planificatorului Airflow (și backfill) va apela această metodă în timpul dezactivării. Orice curățare sincronă necesară pentru a finaliza sarcinile în curs trebuie făcută aici. |
| `terminate` | Oprește mai forțat executorul, oprind/chinuind sarcinile în curs de desfășurare în loc să aștepte sincron completarea lor. |
| `cleanup_stuck_queued_tasks` | Dacă sarcinile sunt blocate în starea de așteptare pentru mai mult timp decât `task_queued_timeout`, acestea sunt colectate de către planificator și furnizate executorului pentru a avea oportunitatea de a le gestiona (efectua orice curățare/grupare elegantă) prin intermediul acestei metode și de a returna instanțele de sarcină pentru un mesaj de avertizare afișat utilizatorilor. |
